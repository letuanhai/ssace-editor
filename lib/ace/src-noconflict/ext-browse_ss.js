...
ace.define("ace/ext/browse_ss", [], function (require, exports, module) {
     ...
     * @property {Function} openItem            Function to open the selected item
    ...
    browse_ss.browse_files = function () {
        browse_ss({
            defaultPath: SsFiles.defaultPath,
    ...
    class SsFiles {
        ...
        static getFileDataItem(filePath) {
            ...
            return fsItem.then(item => {
                ...
                dataItem.children = item.children?.map(item => {
                    const prefix = item.isDirectory ? '📁 ' :
                        item.name?.endsWith('.sas') ? '📘 ' :
                            item.name?.endsWith('.sas7bdat') ? '📅 ' :
    ...
    class SsLibrary {
        ...
        static getStartPath() {
            ...
            return currentTab.type === 'DATA' ?
                currentTab.id.replaceAll('~', '/') + '/'
        ...
        static getLibraryDataItem(libraryItemPath) {
            ...
            return sSItem.then(item => {
                ...
                dataItem.children = item.children?.map(item => {
                    const prefix = item.isLibrary ? (item.engine === "V9" ? '📚️ ' :
                        item.engine === 'ORACLE' ? '📕️ ' : '❔ '
                    ) :
                        item.table ? '📅 ' :
                            item.library === 'columns' ? (item.type === 'Numeric' ? '💯 ' : '©️️ ') :
                                '';
                    return {
                        value: prefix + item.name,
                        prefix: prefix,
                        uri: item.id.replaceAll('~', '/'),
                        keepPrompt: false,
                        meta: (item.isLibrary || item.table) ? '>' : '',
                    };
                });
                dataItem.children?.unshift(
                    {
                        uri: cleanedPath.slice(0, cleanedPath.lastIndexOf('/')),
         ...
         * @param {String} tablePath Absolute file path
         */
        static openTable(tablePath) {
            console.debug('browse_ss: opening table ', tablePath);
            const [root, library, table] = tablePath.split('/');
            const targetItem = {
                type: 'DATA',
        ...
        static childrenItemsCount(childrenItems) {
            ...
            const allCountTexts = [
                libraryCount ? libraryCount + ' libraries' : '',
 ...
 * @property {Partial<SsFileItem>[]?} children Children items
 */
