...
ace.define("ace/ext/browse_ss", [], function (require, exports, module) {
    ...
    window._browseSsTabsListener = function (event) {
        if (event.key !== 't' || !event.altKey || event.ctrlKey || event.metaKey) return;
        event.preventDefault();
        event.stopPropagation();
        browse_ss.browse_tabs();
    }
    window.addEventListener('keydown', window._browseSsTabsListener, true);
    ...
    class SsTabs {
        ...
        static openTab(tabDataItem) {
            console.debug('browse_ss: opening tab ', tabDataItem);
...
function _reloadBrowseSs() {
    ...
    window.removeEventListener('keydown', window._browseSsTabsListener, true);
