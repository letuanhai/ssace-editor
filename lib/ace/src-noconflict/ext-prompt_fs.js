...
ace.define("ace/ext/prompt_fs", [], function (require, exports, module) {
    ...
    function prompt_fs(options) {
        ...
        function accept() {
            ...
            const curData = popup.getData(popup.getRow());
            const cleanedPath = normalizeFilePath(curData.uri || val);
            console.debug('prompt_fs: accepted', val, curData);
    ...
    config.loadModule("ace/commands/default_commands", function (module) {
        module.commands.push({
            ...
            readOnly: true,
            exec: function () {
                config.loadModule("ace/ext/prompt_fs", function (module) {
                    module.prompt_fs();
                })
            }
        });
    ...
    window._promptFsListener = function (/** @type {KeyboardEvent} */ event) {
        if (event.key !== 'p' || !event.altKey || event.ctrlKey || event.metaKey) return;
        event.preventDefault();
        event.stopPropagation();
        prompt_fs();
    }
    window.addEventListener('keydown', window._promptFsListener, true);
    ...
    function getFSItemFromApp(filePath) {
        console.debug('prompt_fs: filePath is ', filePath);
        ...
        console.debug('prompt_fs: got item ', fsItem);
        return fsItem
    }
    ...
    function openFile(filePath) {
        console.debug('prompt_fs: opening file ', filePath);
    ...
    function focusItemOnTree(filePath) {
        console.debug('prompt_fs: focusing tree on item ', filePath);
...
function _reloadPromptFs() {
    const moduleId = 'ace/ext/prompt_fs';
    const srcPath = ace.config.moduleUrl(moduleId);
    window.removeEventListener('keydown', window._promptFsListener, true);
