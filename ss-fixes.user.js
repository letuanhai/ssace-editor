// ==UserScript==
// @name         SAS Studio Fixes
// @namespace    lth
// @version      2024-12-04
// @description  Fix various annoyances and add missing features for SAS Studio
// @author       LTH
// @match        https://*.echonet/SASStudio/*
// @match        https://*.sas.com/SASStudio/*
// @match        http://*/SASStudio/*
// @grant        none
...
/// <reference path="../../browser-ext/SASStudio-3.82/types/globals.d.ts" />
(function () {
    'use strict';

    const ALL_TOOLS = {

        injectConfirmDropFile: function () {
        ...
        injectReloadCurrentFile: function () {
            toolBar.createToolBtn(
                reloadCurrentFile.bind(window.appDMS),
                {
                    textContent: 'Reload file',
...
`
        ...
        injectMiddleClickCloseTab: function () {
            const tabs = window.appDMS.tabs;

            function addCloseTabListener(t) {
                (t.tab ?? t).controlButton.domNode.addEventListener("mouseup", (e) => {
                    if (e.button === 1) {
        ...
        injectScrollTreeToCurrentTabItem: function () {
            toolBar.createToolBtn(
                scrollTreeToCurrentTabItem,
                {
                    textContent: 'Focus tree on current tab item',
        ...
        injectScrollTreeToPath: function () {
            toolBar.createToolBtn(
                scrollTreeToInputPath,
                {
                    textContent: 'Focus tree on input path',
        ...
        injectScrollTreeToSelectedNode: function () {
            toolBar.createToolBtn(
                scrollTreeToSelectedNode,
                {
                    textContent: 'Focus tree on selected node',
        ...
        injectScrollDestinationTreeToProjectSelectedNode: function () {
            toolBar.createToolBtn(
                scrollDestinationTreeToProjectSelectedNode,
                {
                    textContent: 'Scroll destination tree',
        ...
        injectCollapseCurrentTree: function () {
            toolBar.createToolBtn(
                collapseCurrentTree,
                {
                    textContent: 'Collapse current tree',
                    title: 'Collapse all'
                },
                {
                    key: 'F6',
        ...
        injectCloseCurrentTab: function () {
            toolBar.createToolBtn(
                closeCurrentTab,
                {
                    textContent: 'Close current tab',
        ...
        injectReopenClosedTab: function () {
            toolBar.createToolBtn(
                reopenLastClosedTab,
                {
                    textContent: 'Reopen last closed tab',
            ...
            tabs.closeTab = function (tab) {
                if (['FILE', 'DATA', 'IMPORTTOOL'].includes(tab.type)) {
                    const closedTab = {};
                    ['name', 'uri', 'type', 'fileType', 'library'].forEach(
                        (prop) => { closedTab[prop] = tab[prop] }
                    );
        ...
        injectOpenLogInNewTab: function () {
            toolBar.createToolBtn(
                () => {
                ...
                {
                    textContent: 'Open submission log in new tab',
        ...
        injectSelectPreviousTab: function () {
            toolBar.createToolBtn(
                () => selectNextTab(-1),
                {},
                {
        ...
        injectCopyCurrentTabUri: function () {
            toolBar.createToolBtn(
                () => {
                ...
                {
                    textContent: 'Copy current tab URI',
        ...
        injectResetLayoutCurrentTab: function () {
            toolBar.createToolBtn(
                () => resetTabLayout(window.appDMS.tabs.getFocusedTab()),
                {
                    textContent: 'Reset layout current tab',
        ...
        injectResetLayoutAllTabs: function () {
            toolBar.createToolBtn(
                () => {
                    window.appDMS.tabs.getAllTabObjects().forEach(tab => resetTabLayout(tab))
                },
                {
                    textContent: 'Reset layout all tabs',
        ...
        injectSelectPreviousPane: function () {
            toolBar.createToolBtn(
                () => selectNextPane(-1),
                {},
                {
        ...
        injectProjectsContextMenuCopyUri: function () {
            const projects = window.appDMS.projects;
            ...
            projects.populateProjectContextMenu = function () {
                ...
                const copyUriMenuItem = new dijit.MenuItem({
                    label: 'Copy Path',
                    onClick: function () {
                        const selectedItemsUri = window.appDMS.projects.tree
                            .get('selectedItems')?.map?.(i => i.uri);
                        if (selectedItemsUri) {
        ...
        injectTabsContextMenuCopyUri: function () {
            const tabs = window.appDMS.tabs;
            const copyUriMenuItem = new dijit.MenuItem({
                label: 'Copy Path',
                onClick: function () {
                    const currentTargetUri = window.dijit
                        .byNode(this.getParent().currentTarget)?.page?.tabObject?.uri;
                    if (currentTargetUri) {
                        try {
        ...
        injectFocusCodeEditor: function () {
            toolBar.createToolBtn(
                () => {
    ...
    function selectNextTab(n) {
        ...
        const nextTabIndex = (
            (currentTabIndex + (n ?? 1)) % allTabs.length + allTabs.length
        ) % allTabs.length; // wrap array index around
    ...
    function selectNextPane(n) {
        ...
        const allPanes = [];
        [editorTab.sasSuiteTabContainer, editorTab.rightTabs, editorTab.bottomTabs].forEach(
            container => {
                container?.getChildren()
                    .forEach(pane => allPanes.push([pane, container]));
            }
        );
        const currentPaneIndex = allPanes.findIndex(i => i[0].type === editorTab.selectedTab.type);
        const nextPaneIndex = (
            (currentPaneIndex + (n ?? 1)) % allPanes.length + allPanes.length
        ) % allPanes.length; // wrap array index around
    ...
    function reloadCurrentFile() {
        ...
        if (
            ...
            fileUrl = appDMS.baseURL + "/sasexec/sessions/" + appDMS.sessionId + "/" + currentTabType + "/" +
                encodeValue(currentTab.uriParent, isFtp) + "/" + encodedFilePath;
        }
        ...
        if (currentTab.ct) {
            ...
            if (
                "undefined" !== typeof currentTab.asciiEbcdicConversionRequired &&
                true == currentTab.asciiEbcdicConversionRequired
            )
        ...
        dojo.xhrGet({
            ...
            load: dojo.hitch(currentTabEditor, function (data, ioargs) {
                ...
                if (data.length > 1048576 * e) {
                    if (true == appDMS.loaded) {
                        ...
                        this.bigProgramDecisionDialog = appDMS.dialogs.postDecisionDialog(e, null,
                            {
                            ...
                            },
                            {
                                ...
                                callback: dojo.hitch(this, function () {
                                    this.bigProgramDecisionDialog.hide();
                                    this.targetComponent.getParent()
                                        && tabContainer.getParent().removeChild(this.targetComponent);
                                }),
                                primary: true,
                            }
                        );
            ...
            error: dojo.hitch(currentTabEditor, function (error, ioargs) {
                ...
                this.readingProgress &&
                    0 == this.readingProgressCount &&
                    this.readingProgress.hide();
                if (appDMS.loaded)
                    try {
...
- 'ext:' to download a file to open in external programs`,
    ...
    function scrollTreeToCurrentTabItem() {
        const uri = window.appDMS.tabs.getFocusedTab().uri;
        const targetTreeId = uri.startsWith('libraries~') ? 'library' :
            getCurrentTargetTree(['destination', 'projects']).id.split('.')[0];
...
- 'tbl:' followed by a table identifier to open a table from library`,
            ...
            function (userInput) {
                ...
                const targetTreeId = userInput.startsWith('tbl:') ? 'library' :
                    getCurrentTargetTree().id.split('.')[0];
                const userInputPath = userInput.startsWith('tbl:') ?
                    userInput.slice(4) : userInput;
                const targetPath = (targetTreeId === 'library') ?
                    resolveTablePath(userInputPath) :
                    resolveFilePath(userInputPath);
                selectTreePane(targetTreeId);
    ...
    function resolveFilePath(inPath) {
        const rootPath = window.dijit.byId('projects.tree').rootNode.getChildren()
            .filter(
                (node) => node.item.uri != "_folderShortcutsRoot_"
            )[0].item.uri;
    ...
    function getCurrentTargetTree(treeIds, defaultTreeId) {
        ...
        const targetTreeId = allTreeIds.filter(
            (t) => window.dijit.byId(`${t}.tree`)?.domNode?.checkVisibility()
        )?.[0] ?? (defaultTreeId ?? 'projects');
    ...
    /* Buttons inside dropdown content */
    .ssf_dropdown-content button {
        background-color: #555555;
        border: none;
        color: white;
        padding: 6px 8px;
    ...
    .ssf_dropdown-content button:hover {
        background-color:rgb(122, 122, 122);
    }

    /* Show the dropdown content on hover */
    .ssf_dropdown:hover .ssf_dropdown-content {
        display: block;
    }

    /* Change background color of dropdown button on hover */
    .ssf_dropdown:hover .ssf_dropbtn {
     ...
     * @returns {String}
     */
    function getKeyMapName(keymap) {
        if (!keymap?.key) return '';
        let name = keymap.key;
        name = (keymap.altKey ? 'Alt+' : '') + name;
        name = (keymap.metaKey ? 'Meta+' : '') + name;
        name = (keymap.ctrlKey ? 'Ctrl+' : '') + name;
        return ` (${name})`;
    }

    const toolContainer = document.createElement('div');
    toolContainer.className = 'ssf_dropdown';

    const toolContainerBtn = document.createElement('button');
    toolContainerBtn.className = 'ssf_dropbtn';
    toolContainerBtn.textContent = '🛠';
    toolContainerBtn.title = 'Tools';
    toolContainer.appendChild(toolContainerBtn);

    const toolBar = document.createElement('div');
    toolBar.className = 'ssf_dropdown-content';
    toolContainer.appendChild(toolBar);
     ...
     * @param {KeyMap=} keyMap
     */
    toolBar.createToolBtn = function (callback, btn, keyMap) {
        const keyMapName = getKeyMapName(keyMap);
        if (btn?.textContent) {
            const btnEl = document.createElement('button');
            btnEl.textContent = btn.textContent + keyMapName;
            if (btn.title) btnEl.title = btn.title + keyMapName;
            btnEl.onclick = () => callback();
            this.appendChild(btnEl);
        }
        if (keyMap?.key) {
            bindKey(callback, keyMap);
        }
    };

    waitForElm('.dijitTreeNode').then((e) => {
        document.body.appendChild(toolContainer);
        Object.entries(ALL_TOOLS).forEach((t) => { t[1](); });
    });
